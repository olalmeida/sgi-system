        ...
        <div className="budget-view">
            ...
            {loading ? (
                ...
                <div className="empty-state">
                    ...
                    <p className="stat-label">{filterStatus !== 'all' || filterCurrency !== 'all' ? t('finance.adjustFilters') : t('budgets.createFirst')}</p>
                    {filterStatus === 'all' && filterCurrency === 'all' && (
                            ...
                            <div key={budget.id} className="budget-card">
                                    ...
                                    <div>
                                        ...
                                        <p className="stat-label">
                                            {budget.start_date && budget.end_date && (
                                                <>
                                                    {new Date(budget.start_date).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
                                                    {' - '}
                                                    {new Date(budget.end_date).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}
                                                </>
                                            )}
                                        </p>
                                    </div>
                                ...
                                <div className="budget-progress">
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '0.5rem' }}>
                                        <span className="stat-label">{percentage.toFixed(1)}% {t('budgets.spent')}</span>
                                        <span
                                        ...
                                        >
                                            {remaining >= 0 ? (
                                                <>
                                                    ...
                                                    {budget.currency?.symbol || budget.currency_code}
                                                    {remaining.toLocaleString('en-US', { minimumFractionDigits: 2 })} {t('budgets.available')}
                                                </>
                                            ) : (
                                                <>
                                                    ⚠️ {t('budgets.exceeded')} {budget.currency?.symbol || budget.currency_code}
