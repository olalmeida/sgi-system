...
export function LogisticsView() {
    ...
    const handleDrop = async (e: React.DragEvent, newStatus: string) => {
        ...
        if (process && process.status !== newStatus) {
            const { error } = await updateProcess(processId, { status: newStatus as any });
            if (error) {
    ...
    const filteredProcesses = processes.filter(process =>
        !searchTerm || process.name.toLowerCase().includes(searchTerm.toLowerCase())
    );

    const columns = [
        { id: 'pending', title: t('logistics.pending'), icon: <Package size={20} />, color: '#f59e0b' },
        { id: 'in_progress', title: t('logistics.inProgress'), icon: <Truck size={20} />, color: '#3b82f6' },
        { id: 'completed', title: t('logistics.completedStatus'), icon: <Package size={20} />, color: '#10b981' },
        ...
        <div className="logistics-view">
            ...
            {processes.length === 0 ? (
                <div className="empty-state">
                    ...
                    <button className="auth-button" style={{ marginTop: '1rem' }} onClick={() => setShowForm(true)}>
                        <Plus size={18} /> {t('logistics.newProcess')}
                    </button>
                </div>
            ) : (
                    ...
                    {columns.map(column => (
                        ...
                        >
                            <div className="column-header" style={{ borderColor: column.color }}>
                                {column.icon}
                                <h3>{column.title} ({filteredProcesses.filter(p => p.status === column.id).length})</h3>
                            </div>
                                        ...
                                        >
                                            <div className="process-header">
                                                ...
                                                <div className="process-actions">
                                                    <button onClick={(e) => { e.stopPropagation(); handleEdit(process); }} className="icon-button edit">
                                                        <Edit size={16} />
                                                    </button>
                                                    <button onClick={(e) => { e.stopPropagation(); handleDelete(process.id); }} className="icon-button delete">
                                                        <Trash2 size={16} />
                                                    </button>
                                                </div>
                                            </div>
                                            ...
                                            {process.details && (
                                                <div className="process-details-preview">
                                                    {Object.entries(process.details as Record<string, any>).slice(0, 3).map(([key, val]) => (
                                                        <div key={key} className="detail-tag">
                                                            <span className="key">{key}:</span> {String(val)}
                                                        </div>
                                                    ))}
                                                    {Object.keys(process.details as Record<string, any>).length > 3 && (
                                            ...
                                            <div className="process-footer">
                                                <span className="stat-label">
                                                    {new Date(process.created_at).toLocaleDateString(undefined, { day: 'numeric', month: 'short' })}
                                                </span>
                                            </div>
                                        </div>
                                    ))}
